...
\begin{algorithm}
    \caption{Example of a training scheme for \acrlong{vaes}. This algorithm assumes that the encoder posterior probabilities are modeled as a multivariate gaussian distribution with diagonal covariance and the decoder posterior is an isotropic multivariate laplace distribution with known variance. $E(x)$ and $D(z)$ correspond to the encoder and decoder functions. The encoder outputs a vector of mean values and standard deviations for the multivariate gaussian, whereas the decoder only outputs the mean values. $\lambda$ and $\nu$ are taken as hyperparameters.}
    \label{alg:VAEbase}
    \begin{algorithmic}[1]
        \FOR{number of iterations}
        ...
        \STATE Assign parameters for the latent posterior: $\mu_i, \sigma_i \leftarrow E(x_i)$
        \STATE Sample from the posterior and decode it back to the data space: $\hat{x_i} = D(\mu_i + z_i\odot\sigma_i)$, where $\odot$ represents the Hadamarad product.
        \STATE Update the model parameters with the following gradient:
        \begin{equation}
            \nonumber
            \nabla_\theta\left(\frac{1}{m}\sum_{i=1}^m\left\{|\hat{x_i} - x| + \nu(\text{ln}(\sigma_i^2) - \mu_i^2 - \sigma_i^2)\right\}\right).
        \end{equation}
        \ENDFOR
    \end{algorithmic}
\end{algorithm}
