...
Firstly is proven that given a fixed generator $G$ the optimal discriminator is 
\begin{equation}
...
\subsection{Issues with \acrshort{gans}}
...
\subsection{\acrshort{gan} variantions}
...
An issue with training \acrshort{gans} on the original discriminator cost function \ref{eq:GANcost} is that the adversarial term $\log(1-D(G(Z)))$ saturates if the discriminator is too confident. This can prevent the generator from learning in the beginning when the generator output are quite dissimilar to the original data. In the original article it is suggested to train the generator to minimize $-\log(D(G(Z))$ instead. 
